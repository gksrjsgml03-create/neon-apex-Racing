...
window.addEventListener('keydown',e=>{if(['ArrowUp','ArrowDown','ArrowLeft','ArrowRight','Space'].includes(e.code))e.preventDefault();keys.add(e.code);if(e.repeat)return;if(e.code==='Escape'){if(state==='racing')pause();else if(state==='paused'){$('resume').click();}}if(state==='racing'){if(e.code==='KeyE')race.shift();if(e.code==='ShiftLeft'||e.code==='ShiftRight')race.boost();}});
...
function frame(now){const dt=Math.max(0,Math.min((now-last)/1000,.05));last=now;
 if(state==='menu'){race.distance+=dt*2200;race.time+=dt;renderer.draw(race,true);}
  ...
  if(state==='racing'){
   const input={accelerate:keys.has('ArrowUp')||keys.has('KeyW'),brake:keys.has('ArrowDown')||keys.has('KeyS'),left:keys.has('ArrowLeft')||keys.has('KeyA'),right:keys.has('ArrowRight')||keys.has('KeyD'),drift:keys.has('Space')};race.visualSteer=(input.right?1:0)-(input.left?1:0);race.update(dt,input);if(race.finished)finish();
  }
  renderer.draw(race);renderer.map($('map'),race);document.body.classList.toggle('flux',race.flux);
  $('position').innerHTML=`0${race.place}<span> / 06</span>`;$('lap').innerHTML=`LAP ${Math.min(3,Math.floor(race.distance/LENGTH)+1)} <span>/ 3</span>`;$('time').textContent=formatTime(race.time);$('speed').textContent=String(Math.round(race.speed/30)).padStart(3,'0');$('energy').style.width=`${race.energy}%`;$('energy-label').textContent=`페이즈 에너지 ${Math.round(race.energy)}%`;$('drift').style.width=`${race.drift*100}%`;$('phase').textContent=race.flux?'FLUX DIMENSION':'STREET MODE';$('boost-label').textContent=`SHIFT 부스터 × ${race.boosts}`;$('boost-status').textContent=race.boostTime>0?'OVERDRIVE ACTIVE':race.flux?'PHASE DRIVE':'ELECTRIC DRIVE';$('message').textContent=race.countdown>0?(race.countdown>.7?String(Math.ceil(race.countdown-.7)):'GO!'):race.messageTime>0?race.message:'';
 }
 audio.update(race.speed,state==='racing'&&race.countdown===0,race.boostTime>0);requestAnimationFrame(frame);
