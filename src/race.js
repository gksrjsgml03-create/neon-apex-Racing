...
export class Race {
 ...
 boost(){if(this.countdown<=0&&this.boosts>0&&this.boostTime<=0&&!this.finished){this.boosts--;this.boostTime=2.4;this.say('OVERDRIVE');}}
 update(dt,input={}){
  ...
  this.speed=clamp(this.speed+(input.accelerate?2400:-1100)*dt-(input.brake?4800*dt:0),0,max);
  const ratio=this.speed/5700;
  this.x+=steering*dt*(this.drifting?1.35:1.05)*Math.min(1,ratio*2)-curve*ratio*dt*(this.drifting?.29:.43);
  ...
  if(Math.abs(this.x)>edge){this.speed=Math.max(900,this.speed-dt*6500);this.energy=Math.max(0,this.energy-dt*4);}
  this.x=clamp(this.x,-1.55,1.55);
  if(this.flux&&segmentAt(this.distance).fluxZone&&Math.abs(this.x)<.35)this.speed=Math.min(max+1000,this.speed+dt*3800);
  const oldLap=Math.floor(this.distance/LENGTH);
  ...
  if(Math.floor(this.distance/LENGTH)>oldLap){this.lapTimes.push(this.time-this.lapStart);this.lapStart=this.time;this.say(`LAP ${oldLap+2} / 3`);}
