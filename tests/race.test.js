...
test('phase runs out and partial drift charge survives release',()=>{const r=new Race();r.countdown=0;r.energy=.1;r.flux=true;r.update(.1,{});assert.equal(r.flux,false);r.speed=3000;r.drift=.8;r.boosts=2;r.update(.01,{});assert.equal(r.boosts,2);assert.equal(r.drift,.8);});
